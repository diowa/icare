...
RSpec.describe 'Itineraries' do
  ...
  PETS_ALLOWED_ICON = 'span.fas.fa-paw'
  SMOKING_ALLOWED_ICON = 'span.fas.fa-smoking'
  ...
  context 'with registered Users' do
    ...
    it 'allows users to view their own ones' do
      ...
      create :itinerary, user: female, smoking_allowed: true
      create :itinerary, user: female, pets_allowed: true
      ...
      female.itineraries.each do |itinerary|
        ...
        expect(row).to have_css PETS_ICON if itinerary.pets_allowed?
        expect(row).to have_css SMOKING_ICON if itinerary.smoking_allowed?
      end
    end
